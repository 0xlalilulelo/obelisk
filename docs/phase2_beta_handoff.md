...
3. **API service** — deploy this repo (auto-detects `railway.json`). Set `DATABASE_URL` to a reference to the DB service (substitute the real service name — ours is `pgvector` — into the `${{…}}` refs; a wrong name resolves to empty and psycopg falls back to a local socket). Note the `+psycopg` suffix (psycopg3, not psycopg2):
   `postgresql+psycopg://${{pgvector.POSTGRES_USER}}:${{pgvector.POSTGRES_PASSWORD}}@${{pgvector.RAILWAY_PRIVATE_DOMAIN}}:5432/${{pgvector.POSTGRES_DB}}`
   Then paste the rest of `.env.production.example`, including all six flag-flips. Also set **`PORT=8000`** — Railway's healthcheck targets the `PORT` variable and the Dockerfile binds `${PORT:-8000}`; if `PORT` is unset or mismatched you get `service unavailable` healthcheck failures *even though the app starts fine*. Generate a public domain (target **port 8000** too) — that `https://…up.railway.app` URL feeds the **Stripe webhook** endpoint (§1), **iOS `OBELISK_API_URL`** (§3), and **desktop `VITE_API_BASE_URL`** (§4).
4. **Verify** — first deploy's `preDeployCommand` runs the migrations, which create the `vector` extension (`0001_initial.py` → `CREATE EXTENSION IF NOT EXISTS vector`); then run the four checks above. **Healthcheck path is `/v1/healthz`** — the health router mounts under the `/v1` prefix (`main.py`), so a bare `/healthz` 404s and fails the deploy.
